...
final class ImageResize
{
	...
	public static function resize(GDO_File $file, $toWidth, $toHeight, $toFormat = null): true
	{
		...
		{
			case 8:
				$source2 = imagerotate($source, 90, 0);
				break;
			case 6:
				$source2 = imagerotate($source, -90, 0);
				break;
			case 3:
				$source2 = imagerotate($source, 180, 0);
				break;
		}
		if ($source2)
		{
...
// 			case "image/bmp": imagewbmp($desired_gdim, $file->path); break;
			case 'image/gif':
				imagegif($desired_gdim, $file->path);
				break;
			case 'image/jpeg':
				imagejpeg($desired_gdim, $file->path);
				break;
			case 'image/png':
				imagepng($desired_gdim, $file->path);
				break;
            case 'image/webp':
                imagewebp($desired_gdim, $file->path);
                break;
			default:
